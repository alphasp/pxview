...
const styles = StyleSheet.create({
  ...
  bottomTabs: {
    //not working on android error: folly:
    flex: 1,
    backgroundColor: 'transparent',
    position: 'absolute',
    overflow: 'hidden',
...
class Detail extends Component {
  ...
  handleOnChangeTab = ({i, ref}) => {
    ...
    console.log('handleOnChangeTab ', i, selectedBottomTabIndex, selectedBottomTab)
    //if (!selectedBottomTabIndex)
    if (i === selectedBottomTabIndex) {
      ...
      this.refs.bottomTabs.transition({ height: windowHeight - 64, opacity: 0 }, { height: 60, opacity: 1 }, 300);
      InteractionManager.runAfterInteractions(() => {
      ...
      if (!isSwitchingTab) {
        ...
        this.refs.bottomTabs.transition({ height: 60, opacity: 0 }, { height: windowHeight - 64, opacity: 1 }, 300);    
        if (this.refs.imageListContainer) {
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        ...
        {
          !mounting &&
          <Animatable.View style={[styles.bottomTabContainer, bottomTabNewStyle]} ref="bottomTabs">
            <ScrollableTabView 
